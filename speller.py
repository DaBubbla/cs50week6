...
# (e.g., long synonomy for scilicosis)
LENGTH = 45

# Default dictionary
WORDS = "dictionarires/large"
...
# Spell-check each work in file
while True:
    C = file.read(1)
    if not c:
        break

    #Allow alphabetical chars and apostrophes (for possessives)
    if re.match(r"[A-Za-z]", c) or (c == "'" and index > 0):
        ...
        # Ignore words with numbers (like M5 word con)
        elif c.isdigit():
...
if not unloaded:
    print("Count not load $dictionary.")
    sys.exit(1)
